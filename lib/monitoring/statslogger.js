...
StatsLogger.prototype.start = function() {
	...
	this.jtlLog.put('<?xml version="1.0" encoding="UTF-8"?><testResults version="1.2">' + '\n');
...
StatsLogger.prototype.log_ = function() {
    ...
    this.jtlLog.put('<httpSample lt="' + summary.latency.avg + '" ts="' + new Date(summary.ts).getTime() + '" s="true" by="' + summary['response-bytes'].total + '" lb="' + summary.name + '"/>\n');
};
